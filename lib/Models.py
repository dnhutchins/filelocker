...
class User(Base):
    ...
    owned_groups = relation("Group", cascade="all, delete-orphan")
    quota_used = 0
    salt = None
    is_role = False
    authorized = True
    ...
    def get_copy(self):
        ...
        for group in self.groups:
            loadedGroups.append(group.get_copy())
        cUser = User(first_name=self.first_name, last_name=self.last_name, email=self.email, quota=self.quota, date_last_login=self.date_last_login, date_tos_accept=self.date_tos_accept, id=self.id, quota_used=self.quota_used, permissions=loadedPermissions, groups=loadedGroups)
        return cUser
...
class CLIKey(Base):
    ...
    def get_copy(self):
        return CLIKey(user_id=self.user_id, host_ipv4=self.host_ipv4, host_ipv6=self.host_ipv6, value=self.value)

    def get_dict(self):
        return {'user_id': self.user_id, 'host_ipv4': self.host_ipv4, 'host_ipv6': self.host_ipv6, 'value': self.value}
...
#    members = relation("User", secondary=group_membership_table, backref="groups")
    ...
    def get_copy(self):
        ...
        for permission in self.permissions:
            loadedPermissions.append(permission.get_copy())
        return Group(id=self.id, name=self.name, owner_id=self.owner_id, role_owner_id=self.role_owner_id, scope=self.scope, permissions=loadedPermissions)
...
class File(Base):
    ...
    document_type = None
    hidden_shares = relation("HiddenShare", cascade="all, delete-orphan")
...
class HiddenShare(Base):
    ...
    owner_id = Column(String(30), ForeignKey("users.id"), primary_key=True)
...
class UploadRequest(Base):
    ...
    scan_file = Column(Boolean, nullable=False, default=True)
    notify_user = Column(Boolean, nullable=False)
    date_expires = Column(DateTime)
    password = Column(String(80))
    ...
    def get_copy(self):
        return UploadRequest(id=self.id, owner_id=self.owner_id, max_file_size=self.max_file_size, scan_file=self.scan_file, notify_user=self.notify_user, date_expires=self.date_expires, password=self.password, type=self.type)
