...
class AccountController:
    ...
    def create_user(self, userId, firstName, lastName, email, quota, format="json", requestOrigin="", **kwargs):
    ...
    @cherrypy.expose
    @cherrypy.tools.requires_login()
    def create_group(self, groupName, groupMemberIds=None, groupScope="private", format="json", requestOrigin="", **kwargs):
        user, sMessages, fMessages = (cherrypy.session.get("user"), [], [])
        if requestOrigin != cherrypy.session['request-origin']:
            fMessages.append("Missing request key!!")
        else:
    ...
    @cherrypy.expose
    @cherrypy.tools.requires_login(permission="admin")
    def update_role(self, roleId, roleName, email, quota, format="json", requestOrigin="", **kwargs):
        user, sMessages, fMessages = (cherrypy.session.get("user"), [], [])
        if requestOrigin != cherrypy.session['request-origin']:
            fMessages.append("Missing request key!!")
        else:
    ...
    @cherrypy.expose
    @cherrypy.tools.requires_login()
    def switch_roles(self, roleId=None, format="json", requestOrigin="", **kwargs):
        user, sMessages, fMessages = (cherrypy.session.get("user"), [], [])
        if requestOrigin != cherrypy.session['request-origin']:
            fMessages.append("Missing request key!!")
        else:
    ...
    @cherrypy.expose
    @cherrypy.tools.requires_login(permission="admin")
    def add_users_to_role(self, roleId, userIds, format="json", requestOrigin="", **kwargs):
        user, sMessages, fMessages = (cherrypy.session.get("user"), [], [])
        if requestOrigin != cherrypy.session['request-origin']:
            fMessages.append("Missing request key!!")
        else:
    ...
    @cherrypy.expose
    @cherrypy.tools.requires_login(permission="admin")
    def create_attribute(self, attributeName, attributeId, format="json", requestOrigin="", **kwargs):
        user, sMessages, fMessages = (cherrypy.session.get("user"), [], [])
        if requestOrigin != cherrypy.session['request-origin']:
            fMessages.append("Missing request key!!")
        else:
    ...
    @cherrypy.expose
    @cherrypy.tools.requires_login(permission="admin")
    def grant_user_permission(self, userId, permissionId, format="json", requestOrigin="", **kwargs):
        user, sMessages, fMessages = (cherrypy.session.get("user"), [], [])
        if requestOrigin != cherrypy.session['request-origin']:
            fMessages.append("Missing request key!!")
        else:
    ...
    @cherrypy.expose
    @cherrypy.tools.requires_login(permission="admin")
    def grant_role_permission(self, roleId, permissionId, format="json", requestOrigin="", **kwargs):
        user, sMessages, fMessages = (cherrypy.session.get("user"), [], [])
        if requestOrigin != cherrypy.session['request-origin']:
            fMessages.append("Missing request key!!")
        else:
    ...
    @cherrypy.expose
    @cherrypy.tools.requires_login()
    def revoke_user_permission(self, userId, permissionId, format="json", requestOrigin="", **kwargs):
        user, sMessages, fMessages = (cherrypy.session.get("user"), [], [])
        if requestOrigin != cherrypy.session['request-origin']:
            fMessages.append("Missing request key!!")
        else:
    ...
    @cherrypy.expose
    @cherrypy.tools.requires_login()
    def revoke_role_permission(self, roleId, permissionId, format="json", requestOrigin="", **kwargs):
        user, sMessages, fMessages = (cherrypy.session.get("user"), [], [])
        if requestOrigin != cherrypy.session['request-origin']:
            fMessages.append("Missing request key!!")
        else:
    ...
    def get_clikey_list(self, format="json", **kwargs):
        user, sMessages, fMessages = (cherrypy.session.get("user"), [], [])
        try:
            CLIkeysQuery = session.query(CLIKey).filter(CLIKey.user_id==user.id)
        except Exception, e:
            fMessages.append("Could not get keys: %s" % str(e))
        CLIkeys = CLIkeysQuery.all()
        return fl_response(sMessages, fMessages, format, data=CLIkeys)
    ...
    def download_CLIconf(self, CLIKey, **kwargs):
        ...
        try:
            ...
            response.body = str(Template(file=get_template_file('filelocker_cli.conf.tmpl'), searchList=[locals(),globals()]))
            response.headers['Content-Length'] = len(response.body[0])
            response.stream = True
            return response.body
        except Exception, e:
            cherrypy.log.error("[%s] [download_cliconfig_file] [Unable to serve CLI configuration: %s]" % (user.id, str(e)))
            raise cherrypy.HTTPError(500, "Unable to serve CLI configuration: %s" % str(e))
